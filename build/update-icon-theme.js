...
const FROM_DISK = true; // set to true to take content from locally; false from original online repo
...
if (!FROM_DISK) {
	...
	fontMappingsFile = './styles/_fonts/seti.less';
	fileAssociationFile = './styles/components/icons/mapping.less';
	colorsFile = './styles/ui-variables.less';
}
...
exports.copyFont = function () {
	return downloadBinary(font, './out/seti.woff');
};
...
exports.update = function () {
	...
	function writeFileIconContent(info) {
		...
		const path = './out/icon-theme.json';
			...
			return download(colorsFile).then(function (content) {
				...
				return getCommitSha('jesseweed/seti-ui').then(function (info) {
					try {
						...
						const cgmanifestPath = './build/cgmanifest.json';
